...
**********************************************************************************/
UINT32 Crc32(const BYTE *puchAddress, INT32 lLength)
{
	...
	{
		lLength = lLength - 1;
		ulCrc = ((ulCrc >> 8) & 0x00FFFFFF) ^ ulCRC32Table[(ulCrc ^ *puchAddress) & 0xFF];
		puchAddress = puchAddress + 1;
		//CLRWDT();
	}
